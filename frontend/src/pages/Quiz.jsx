...
const Quiz = () => {
    ...
    useEffect(() => {
        ...
        if (!storedUserInfo || !storedPreTestScore) {
             navigate('/quiz'); // Garante que se os dados não existirem, volta para o início do fluxo
        }
        setUserInfo(JSON.parse(storedUserInfo));
        setPreTestScore(parseInt(storedPreTestScore, 10));
    }, [navigate]);
            ...
            <div className="min-h-screen bg-gray-50 py-8">
                <div className="max-w-4xl mx-auto px-4">
                    <Card className="text-center">
                        <CardHeader><Trophy className="mx-auto h-12 w-12 text-yellow-500" /><CardTitle className="text-3xl">Resultado Final, {userInfo?.name}!</CardTitle></CardHeader>
                        <CardContent className="space-y-6">
                            <div className="flex justify-around items-center p-4 bg-gray-50 rounded-lg">
                                <div><p className="text-sm text-gray-500">Resultado Anterior</p><p className="text-4xl font-bold text-gray-600">{preTestScore}/{quizQuestions.length}</p></div>
                                <ArrowRight className="h-8 w-8 text-green-500" />
                                <div><p className="text-sm text-green-600">Resultado Final</p><p className="text-5xl font-bold text-green-600">{score}/{quizQuestions.length}</p></div>
                            </div>
                                ...
                                {quizQuestions.map((q, index) => {
                                    ...
                                    return (
                                        <div key={q.id} className={`p-4 rounded-lg border-2 ${userAnswer.isCorrect ? 'border-green-300 bg-green-50' : 'border-red-300 bg-red-50'}`}>
                                            ...
                                            <p className={`mt-2 ${userAnswer.isCorrect ? 'text-green-800' : 'text-red-800'}`}>Sua resposta: {userChoice.text} {userAnswer.isCorrect ? <CheckCircle className="inline h-5 w-5"/> : <XCircle className="inline h-5 w-5"/>}</p>
    ...
    return (
        <div className="min-h-screen bg-gray-50 py-8">
            <div className="max-w-4xl mx-auto px-4">
                 ...
                 <Card>
                    <CardHeader><Badge variant="secondary" className="w-fit">{question.category}</Badge><CardTitle className="text-xl pt-4">{currentQuestion + 1}. {question.question}</CardTitle></CardHeader>
                    <CardContent>
                        <div className="space-y-3">
                            {question.options.map((option) => (<Button key={option.id} variant={selectedAnswer?.id === option.id ? "default" : "outline"} className="w-full justify-start p-4 h-auto text-left" onClick={() => handleAnswerSelect(option)} disabled={showFeedback}>{option.text}</Button>))}
                        </div>
                        {showFeedback && (
                 ...
                 <div className="flex justify-end mt-6">
                    {!showFeedback ? (<Button onClick={handleConfirmAnswer} disabled={!selectedAnswer}>Confirmar</Button>) : (<Button onClick={handleNextQuestion}>{currentQuestion === quizQuestions.length - 1 ? 'Ver Resultado Final' : 'Próxima'} <ArrowRight className="ml-2 h-4 w-4" /></Button>)}
                 </div>
            </div>
        </div>
    );
